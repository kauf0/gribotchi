...
describe('склонение по числу', () => {
  /**
   * Реестр подставляет число в живую фразу, и «ещё 4 штаммов» выдаёт машину
   * сразу. Проверяется вся тройка форм плюс десятки — они и есть ловушка:
   * 11-14 оканчиваются на 1-4, а форма у них общая.
   */
  const shtamm = (n: number) => plural(n, 'штамм', 'штамма', 'штаммов')
  ...
  it.each([
    ...
    expect(shtamm(n)).toBe(form)
  })

  it('ноль — множественное: «ещё 0 штаммов» не пишется, но форма верна', () => {
    expect(shtamm(0)).toBe('штаммов')
  })

  it('строки реестра склоняются, а не приписывают «шт.»', () => {
    expect(REGISTRY.needStrains(1)).toBe('1 штамм')
    expect(REGISTRY.needNames(2)).toBe('2 имени')
    expect(REGISTRY.needTraits(5)).toBe('5 признаков')
  })
})
